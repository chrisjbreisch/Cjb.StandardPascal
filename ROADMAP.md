...
- [x] Implement arrays, multidimensional arrays, packed character arrays, records, and variant fields.
...
- [x] Complete ISO fixed-array character/string semantics and compatibility rules.
- [x] Implement record-field access, array indexing, pointer dereference, and file operations including `read` and `readln`.
- [x] Define structured runtime values, copy/reference semantics, aliasing, bounds checks, set limits, file errors, and pointer lifetime errors.
...
- Variant-record alternatives expose their declared scalar fields through the same record field storage as fixed fields.
- Arrays, records, files, and pointers use reference aliasing on assignment. Packed character arrays accept exact-length strings; array subscripts, file reads, and pointer lifetime operations preserve source-correlated errors.
